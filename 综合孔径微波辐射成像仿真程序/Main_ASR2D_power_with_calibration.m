...
% 图片导入：（仅针对平面阵）
 ...
 %%%%%%%%%%%%%%%%%%校正场景定义（二维）%%%%%%%%%%%%%%%%%%%%         
% 单辅助源幅度相位误差校正
  CAS_param.idealpoint_simu = 1;      %是否仿真，0表示不仿真，1表示仿真
  CAS_param.idealpoint_place = [0;0];   %校正源真实来波方向，度
  CAS_param.nomipoint_place = [0;0];    %校正源标称来波方向，度
  CAS_param.idealpoint_power = [1000]; %源的能量（K）
...
%通道同相相位误差大小
SRM_param.channel_info.error_Iphase_quantity  = [0 pi];
%通道正交相位误差类型
...
IAM_param.inverse_name = InverseAlgorithmSelect(sys_param.inverse_name,sys_param.array_type); 
% 通过可见度进行反演
inv_T = IAMSimulate(Calibrated_self_correlation_matrix,SRM_param,IAM_param);
